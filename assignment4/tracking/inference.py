...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        '''
        This method correctly update the agent's belief distribution over ghost positions given an observation from Pacman's sensors. It is implementing the online belief update for observing new evidence by updating the belief at every position on the map after receiving a sensor reading. It iterates its updates over the variable self.allPositions which includes all legal positions plus the special jail position. Beliefs represent the probability that the ghost is at a particular location, and are stored in a field called self.beliefs, which is being updated here.
        '''
        ...
        '''
        When we capture a ghost and send it to the jail location, our distance sensor deterministically returns None, and nothing else. So, if the ghost's position is the jail position, then the observation is None with probability 1, and everything else with probability 0. Conversely, if the distance reading is not None, then the ghost is in jail with probability 0. If the distance reading is None, then the ghost is in jail with probability 1.
        '''
        if noisyDistance is None:
            # then reinitialized all of the dictionary's position's probability values except the jail position's one to be 0
            allPossible = util.Counter()
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        '''
        This method updates the belief at every position on the map after one time step elapsing.
        '''
        ...
        for oldPos in self.legalPositions:
    	    newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                #print "oldPos: ", oldPos, "oldProb: ", self.beliefs[oldPos], "newPos: ", newPos, "newProb: ", prob
                allPossible[newPos] += prob * self.beliefs[oldPos]
                #print "latestProb: ", allPossible[newPos]
